...
my($frqfile)=""; my($ref_annot)=""; my($annot)=""; my($nb_quantile)=5; my($maf_threshold)=0.05; my($out_file)=""; my($i); my($j); my($k); my($printhelp); my($exclude0);
GetOptions(
  "help"            => \$printhelp,
  "frqfile=s"   => \$frqfile, 
  "ref-annot=s" => \$ref_annot, 
...
if (($frqfile eq "") || ($ref_annot eq "") || ($annot eq "") || ($out_file eq "")) {die "\nERROR! --freqfile, --ref-annot, --annot-header and --out options are mandatory.\n"}

#Step 0: find annot in ref-ld
my($annot_file)=""; 
my($annot_colm)="";
my($total_nb_annotation)=0;
my(@list_ref_annot)=split(/,/, $ref_annot);
my($nb_ref_annot)  =$#list_ref_annot+1;
print "ref-ld   : $nb_ref_annot file(s)\n";
for ($i=0; $i<=$#list_ref_annot ; $i++) {
	print "               $list_ref_annot[$i]\n"; 
	my $IN = IO::Uncompress::Gunzip->new( " $list_ref_annot[$i].annot.gz" ) or die "IO::Uncompress::Gunzip failed: $GunzipError\n";
	my(@line)=split ' ', <$IN>;
	for ($j=0; $j<=$#line ; $j++) {
		if($line[$j] eq $annot){
	...
	close $IN;
	$total_nb_annotation=$total_nb_annotation+$#line;
}
...
#Step 1: Stock MAF from $frqfile files
print "Step 1: Stock MAF from $frqfile files\n";
my(@MAF)=();
my($chr);
my($nbSNPs)=0;
my($nbcommonSNPs)=0;
for ($chr=1; $chr<=1 ; $chr++) {	
	if (-e "$frqfile.frq") {
		open(IN,"$frqfile.frq") or die "\nERROR! Cannot open $frqfile.frq.\n";
		while (<IN>) {
		...
		close IN;
	} elsif (-e "$frqfile.frq.gz")  {
		my $IN = IO::Uncompress::Gunzip->new( "$frqfile.frq.gz" ) or die "IO::Uncompress::Gunzip failed: $GunzipError\n";
		while (<$IN>) {
			chomp $_; my(@line)=split;
			if($line[4] ne "MAF"){
				if ($line[4]>=$maf_threshold && $line[4]<=(1-$maf_threshold)) { push(@MAF,1); $nbcommonSNPs++} else { push(@MAF,0) };
		...
		die "\nERROR! Cannot open $frqfile.frq(.gz).\n";
	}
}
...
$j=$annot_colm-1;
for ($chr=1; $chr<=1 ; $chr++) {
	my $IN = IO::Uncompress::Gunzip->new( " $annot_file.annot.gz" ) or die "IO::Uncompress::Gunzip failed: $GunzipError\n";
	while (<$IN>) {
		chomp $_; my(@line)=split;
		if($line[$j] ne $annot){
			if ($MAF[$cpt]==1){ 
...
for ($k=0; $k<=$#list_ref_annot ; $k++) {
	...
	for ($chr=1; $chr<=1 ; $chr++) { 
		my($cpt0)=0;
		my $IN = IO::Uncompress::Gunzip->new( " $list_ref_annot[$k].annot.gz" ) or die "IO::Uncompress::Gunzip failed: $GunzipError\n";
		while (<$IN>) {
			chomp $_; my(@line)=split;
			if($cpt0>0){
				if ($MAF[$cpt]==1){
